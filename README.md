...
This fork tracks progress locally instead of via the upstream frogress badges
(those only reflect the public Xeeynamo/sotn-decomp repo, not this fork's
local branches). Ask Claude for a "point d'etape" / status report and it
will compute real numbers from the working tree.
