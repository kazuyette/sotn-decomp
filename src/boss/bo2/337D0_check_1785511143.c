...
// NOT an empty function -- 0x801B5C2C..0x801B6930+ is a huge all-zero-word
// BSS scratch block (splat's raw asm dump for this range is page after page
// of `nop`, i.e. 0x00000000 words with no `jr $ra` anywhere; confirmed via
// repo-wide grep that this address is never `jal`'d, only read/written as a
// pointer by create_entity.c). splat's classifier defaulted the very first
// word to a bogus "function" named with the func_us_ prefix since it had no
// data-symbol hint for that exact word (unlike the D_us_-prefixed symbols a
// few words later in the same block, which it got right). Kept the
// pre-existing splat symbol name (func_us_801B5C2C) rather than renaming it,
// since create_entity.c's own disassembly already references this exact
// name as data throughout -- renaming would just require re-deriving every
// call site there for no benefit. See bo2.h.
u16* func_us_801B5C2C;
